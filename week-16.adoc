...
Data importer also stores the current mapping data in the browser session, so if you open the app again later, the mapping is still there, you just need to reload the data files.
...
So we can log in to Neo4j Browser and run the following statement to extract Genre nodes and connect the movie to its generes.
...
The denormalized dataset, which contains all data in a single file.

Usually you get denormalized when a data export from a relational database joined all tables together in one file or a data scientist exported a single data-frame that joined all data together.
